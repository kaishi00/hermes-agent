...
summary/result/metadata, kanban_block reason, kanban_request_review
summary/reason/metadata, and kanban_request_changes reason are masked
before the values reach the DB.  Uses the same worker_env fixture
pattern as test_kanban_tools.py.
"""
...
# ---------------------------------------------------------------------------
# Review-handoff redaction tests — kanban_request_review
# ---------------------------------------------------------------------------

def test_kanban_request_review_summary_scrubbed(worker_env):
    """GitHub PAT in request_review summary must be masked before DB write."""
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb
    secret = "ghp_" + "E" * 40
    kt._handle_request_review({"summary": f"done, key={secret}"})
    conn = kb.connect()
    try:
        run = kb.latest_run(conn, worker_env)
    finally:
...
def test_kanban_request_review_reason_scrubbed(worker_env):
    """OpenAI key in request_review reason must be masked before DB write."""
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb
    secret = "sk-" + "F" * 48
    kt._handle_request_review({"reason": f"token={secret}"})
    conn = kb.connect()
    try:
        run = kb.latest_run(conn, worker_env)
    finally:
...
def test_kanban_request_review_metadata_scrubbed(worker_env):
    """Secret in request_review metadata dict must be masked in stored JSON."""
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb
    secret = "ghp_" + "G" * 40
    metadata = {"diff_path": "/repo/x.py", "api_key": secret}
    kt._handle_request_review({"summary": "ready", "metadata": metadata})
    conn = kb.connect()
    try:
        run = kb.latest_run(conn, worker_env)
    finally:
...
# ---------------------------------------------------------------------------
# Review-handoff redaction tests — kanban_request_changes
# ---------------------------------------------------------------------------

def test_kanban_request_changes_reason_scrubbed(worker_env):
    """Secret in request_changes reason must be masked before DB write.

    request_changes is reviewer-facing: the task must be in 'review'
    status first, so we transition it via request_review before testing.
    """
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb

    # First submit for review so the task is in 'review' status
    kt._handle_request_review({"summary": "implementer done"})

    # Now claim it as a reviewer and request changes
    conn = kb.connect()
    try:
    ...
    secret = "sk-" + "H" * 48
    kt._handle_request_changes({"reason": f"fix token={secret}"})
    conn = kb.connect()
    try:
        # request_changes stores reason in the changes_requested event
        events = kb.list_events(conn, worker_env)
    finally:
        conn.close()
    event_payloads = json.dumps([e.payload for e in events if e.payload])
